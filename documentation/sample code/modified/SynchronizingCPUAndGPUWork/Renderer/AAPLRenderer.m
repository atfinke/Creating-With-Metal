...
- (void)generateTriangles
{
    ...
    for(NSUInteger t = 0; t < NumTriangles; t++)
    {
        ...
        triangle.origin = triangle.position;
...
- (void)updateState
{
    ...
    for(NSUInteger triangle = 0; triangle < NumTriangles; triangle++)
    {
        ...
        vector_float2 triangleOrigin = _triangles[triangle].origin;
        
        
        simd_float4 color = _triangles[triangle].color;
        
        float range = 0.2;
        float subtractor = ((float) range) / 2.0;
        int intRange = range * 10000;
        float red = (float) arc4random_uniform(intRange) / 10000 - subtractor;
        float green = (float) arc4random_uniform(intRange) / 10000 - subtractor;
        float blue = (float) arc4random_uniform(intRange) / 10000 - subtractor;
        
        _triangles[triangle].color = simd_make_float4(color[0] + red, color[1] + green, color[2] + blue,1);
        

        // Displace the y-position of the triangle using a sine wave.
        trianglePosition.y = (sin(triangleOrigin.x/waveMagnitude + _wavePosition) * waveMagnitude);
        
        trianglePosition.x = triangleOrigin.x + (sin(triangleOrigin.x/waveMagnitude + _wavePosition) * waveMagnitude);
